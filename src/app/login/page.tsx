...
export default function LoginPage() {
  ...
  // simple controlled inputs (so later we can do real auth)
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    toast.success(isLogin ? "Welcome back!" : "Account created!", {
      description: isLogin ? "You are now logged in." : "You can start booking and shopping.",
    });
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        ...
        >
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <Button variant="hero" className="w-full rounded-full py-6 text-base mt-2" type="submit">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              className="flex items-center justify-center gap-2 py-3 rounded-xl border border-border bg-card hover:bg-secondary text-sm font-body font-medium transition-colors"
              onClick={() => toast("Google login (mock)", { description: "Hook this later." })}
            >
              Google
            </button>
            <button
              type="button"
              className="flex items-center justify-center gap-2 py-3 rounded-xl border border-border bg-card hover:bg-secondary text-sm font-body font-medium transition-colors"
              onClick={() => toast("Apple login (mock)", { description: "Hook this later." })}
            >
              Apple
            </button>
          </div>
